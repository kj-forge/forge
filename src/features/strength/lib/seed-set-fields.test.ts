...
describe("seedSetFields", () => {
  test("today's set of the kind wins over history", () => {
    ...
    expect(seedSetFields(sets, lastByKind, "TOP_SET")).toEqual({
      reps: 5,
      weightKg: 132.5,
  ...
  test("multiple sets of the kind → the latest one", () => {
    ...
    expect(seedSetFields(sets, {}, "WARMUP")).toEqual({ reps: 8, weightKg: 80, durationSeconds: undefined });
  });

  test("no set of the kind today → falls back to history", () => {
    const sets = [{ kind: "WARMUP", reps: 10, weightKg: 60 }];
    const lastByKind = { TOP_SET: { reps: 3, weightKg: 130 } };
    expect(seedSetFields(sets, lastByKind, "TOP_SET")).toEqual({ reps: 3, weightKg: 130, durationSeconds: undefined });
  });
  ...
  test("bodyweight session set: null weight → 0", () => {
    ...
    expect(seedSetFields(sets, {}, "WARMUP")).toEqual({ reps: 12, weightKg: 0, durationSeconds: undefined });
  });

  test("bodyweight history ref: null weight → 0, null reps → undefined", () => {
    const lastByKind = { WARMUP: { reps: null, weightKg: null } };
    expect(seedSetFields([], lastByKind, "WARMUP")).toEqual({
      reps: undefined,
      weightKg: 0,
      durationSeconds: undefined,
    });
