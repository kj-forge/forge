...
6. **`athlete_id NOT NULL` on every owned table**, with `.references(() => athletes.id, { onDelete: "cascade" })` and an index that starts with `athleteId`. This is the multi-tenant invariant from ADR-0010, made concrete. Tables that are not owned (catalogs like `hyrox_stations`, `progression_rules`, `rehab_protocols`; auth principals like `users`, `coaches`; link tables) do not carry `athlete_id`. *(Amended by [ADR-0020](ADR-0020-per-user-exercise-catalogue.md): `exercises` moved from this catalog class to athlete-owned rows with NULL-athlete_id templates.)*
