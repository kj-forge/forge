...
- [x] Add `isMainLift: boolean().notNull().default(false),` to `exercises`.
- [x] `bun run db:generate`, then append `UPDATE exercises SET is_main_lift = true WHERE slug IN (...four slugs...);` as a custom statement in the generated migration (drizzle custom SQL) — verify slug spelling against seed data first.
- [x] `bun run format` (drizzle snapshot vs biome). Run `bun run db:migrate` on dev DB.
- [x] Export `ACCESSORY_SLUGS: string[]` in `src/features/strength/constants.ts` (rdl, bulgarian split squat, weighted pull-up, dips slugs from seed).
- [x] Verify + commit `feat(db): is_main_lift flag on exercises`.
...
- [x] RED: tests (Epley known values: 100×1=100, 100×5≈116.5, rounding; bestSet ordering/warmup exclusion/bw-only → null; isNewPR edges incl. equal e1RM → false). Run, watch fail.
- [x] GREEN: implement; full suite green; commit `feat(strength): e1rm and pr-detection libs`.
...
- [x] TDD the formatter, then implement server fns (athlete-scoped, zod inputValidator). Commit `feat(strength): stats server fns + compact set formatter`.

### Task 4: Nav — Statystyki tab

**Files:** Modify `src/shared/lib/nav.ts` + `nav.test.ts`.

- [x] RED: update tests — five tabs `["/", "/sessions", "/sessions/new", "/stats", "/me"]` (Nowa stays center-ish: order `/, /sessions, /sessions/new, /stats, /me`), labels `[..., "Statystyki", ...]`, `showsTabBar("/stats")` true, `"/stats/abc"` false.
- [x] GREEN: add `{ to: "/stats", label: "Statystyki", icon: ChartNoAxesColumn, exact: true }` + TAB_BAR_PATHS entry; tab grid `grid-cols-4` → `grid-cols-5` in `AppShell.tsx`.
- [x] Commit `feat(ui): statystyki nav tab`.
...
- [x] Segment control (Rekordy | Zestawienia) — gradient-active chips like the drawer kind chips.
- [x] Rekordy: PR rows — name + record date left; right: heaviest set (ember 900 tabular) over `e1RM ~X kg` muted; NOT links (drill-down dropped). `+ Akcesoria` toggle → search param + localStorage default; accessories section under a small divider.
- [x] Zestawienia: day chips PON–ND (default = today's weekday); matrix table in `overflow-x-auto` container — sticky first column (exercise names, `bg-card2`), one column per session date (newest first, newest header+cells ember), cells via `formatSetsCompact`, `—` when the exercise is absent; desktop full width, mobile horizontal scroll.
- [x] Empty states for both segments; both themes; dev smoke; commit `feat(strength): stats view with rekordy and zestawienia`.
...
- [x] Server part first (pure compare unit-tested in Task 2), then Toaster + call site. No other form logic changes.
- [x] Verify save flow on dev (beat a PR → toast, non-PR → silence) + commit `feat(strength): pr celebration toast on record sets`.

### Task 7: Docs (Phase E) + handoff

- [x] `docs/learning/estimated-1rm-and-pr-detection.md` (Epley vs alternatives, why e1RM for comparing rep ranges, PR semantics with warmups/bodyweight).
- [x] ADR only if a non-obvious architectural call emerged (flag-on-exercises vs config table — one paragraph in learning doc otherwise).
- [x] Update this plan's checkboxes; commit `docs: stats epic learning notes`.
