...
| `session_segments` | Hyrox live timeline (Stage 2, implemented): one row per coach-tapped station/rox-zone/rest interval. `STATION` rows additionally write a mirror row into `sets` (same transaction) so exercise history/PR pipelines need zero Hyrox-specific code. See ADR-0023 and [`docs/learning/hyrox-live-timing.md`](../learning/hyrox-live-timing.md). | id, athlete_id (denormalized), session_id, block_id, round_number, order_index, kind (`STATION` \| `ROX_ZONE` \| `REST`), block_movement_id (nullable — required iff `kind = STATION`, enforced in Zod not a DB check), duration_ms, created_at. Unique index on `(block_id, round_number, order_index)` makes `saveHyroxSegments`'s `onConflictDoNothing` insert idempotent under retry. |
